...
jQuery(function ($) {
    ...
    var $clearPushDateBtn = $('#clear-push-date');

    console.log('%cCopro %cShedule%c Algorithm!%c...\n\n', 'color:red;font-size:34px', 'color:blue;font-size:30px', 'color:green;font-size:25px', 'color:pink;font-style:italic;font-size:16px');
    ...
    function createModalOnForm() {
        ...
        $form.find('button[type="submit"]').on('click', function (evt) {
            var activatePushDateChecked = $('[name="activate-push-date"]', $form).is(':checked');
            var honorPinnedChecked = $('[name="honor-pinned-meetings"]', $form).is(':checked');
            var overwriteFinalizeChecked = $('input[name="finalize"]').length && $('input[name="finalize"]').is(':checked');
            var calendarId = $('input[name="calendar-id"]').val();
            if (!overwriteFinalizeChecked) {
                console.log('%cform %cwas %csubmitted!', 'font-style:italic;font-weight:bold;font-size:40px;color:green', 'font-size:25px;color:blue', 'font-size:30px;color:red');
            ...
            console.log('%cSHOWING %cCONFIRMATION %cMODAL!!!', 'font-style:italic;font-weight:bold;font-size:40px;color:green', 'font-size:25px;color:blue', 'font-size:30px;color:red');

            $modal.modal('show');

            $('button[data-run-form="true"]', $modal).one('click', function () {
                $modal.modal('hide');

                var $loadingAnime = $('.loading-animation');
